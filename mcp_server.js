...
function loadConfig() {
    ...
    if (!raw.port)
        throw new Error(`'port' is required in ${configPath}`);
    return {
...
function httpGet(hostname, port, urlPath, extraHeaders, timeoutMs = 10_000) {
    return new Promise((resolve, reject) => {
        const req = http.request({ hostname, port, path: urlPath, method: "GET", headers: extraHeaders, timeout: timeoutMs }, (res) => {
            let raw = "";
            res.on("data", (c) => { raw += c; });
            res.on("end", () => {
                try {
...
function httpPost(port, urlPath, body, timeoutMs = 120_000) {
...
function apiGet(urlPath, timeoutMs = 10_000) {
...
function toContainerPath(filePath) {
    ...
    if (!p.match(/^[A-Za-z]:/) && !p.startsWith("/source/")) {
        ...
        if (hostRoot)
            return toContainerPath(hostRoot + "/" + p);
    }
...
async function queueWarning() {
    try {
        ...
        const parts = [];
        if (depth > 0)
            parts.push(`${depth} files queued`);
        if (syncerRunning)
            parts.push("syncer walk in progress");
        if (parts.length)
            return `[WARNING: index has outstanding work — ${parts.join(", ")}. Results may be incomplete.]\n\n`;
    }
...
Examples:
  ...
  query_codebase("declarations", "SaveChanges", symbol_kind="method")`, {
    ...
    if (LISTING.has(m)) {
        return { content: [{ type: "text", text: `Mode '${m}' lists file contents without filtering — use query_single_file instead:\n  query_single_file("${m}", file="$SRC_ROOT/path/to/File.cs")` }] };
    }
    let result;
    try {
        ...
        return { content: [{ type: "text", text: `Could not reach indexserver: ${e.message}\nStart it with: ts start` }] };
    }
    const warn = await queueWarning();
    if (result.status === 503 && result.data?.loading) {
        return { content: [{ type: "text", text: "Typesense is still starting up — retry in a few seconds.\nUse service_status() to check when it is ready." }] };
    }
    if (result.status >= 400) {
        ...
        if (detail)
            msg += `\nDetail: ${detail}`;
        return { content: [{ type: "text", text: warn + msg }] };
    }
    ...
    if (!hits.length)
        return { content: [{ type: "text", text: warn + header + "No AST matches found." }] };
    const outLines = [];
    for (const hit of hits) {
        const rel = hit.document?.relative_path ?? "";
        for (const match of hit.matches ?? [])
            outLines.push(`${rel}:${match.line}: ${(match.text ?? "").trimEnd()}`);
    }
    ...
    if (output.length > MAX_OUTPUT_CHARS) {
        ...
        return { content: [{ type: "text", text: warn + header + summary + (nl > 0 ? trunc.slice(0, nl) : trunc) }] };
    }
    return { content: [{ type: "text", text: warn + header + output }] };
});
// ── query_single_file ─────────────────────────────────────────────────────────
server.tool("query_single_file", `Run a tree-sitter AST query on a single file. No Typesense search.
...
Examples:
  ...
  query_single_file("methods", file="$SRC_ROOT/Core/BigFile.cs", offset=250)`, {
    ...
    if (!file)
        return { content: [{ type: "text", text: "file= is required." }] };
    let srcRoot;
    try {
        [, srcRoot] = getRoot(root);
    }
    catch (e) {
    ...
    if (result.status >= 400) {
        return { content: [{ type: "text", text: `Query failed (${result.status}): ${result.data?.error ?? JSON.stringify(result.data)}` }] };
    }
    ...
    if (!fileResult?.matches?.length)
        return { content: [{ type: "text", text: header + "No matches found." }] };
    const allLines = fileResult.matches.map((m) => `${rel}:${m.line}: ${(m.text ?? "").trimEnd()}`);
    ...
    if (output.length > MAX_OUTPUT_CHARS) {
        ...
        return { content: [{ type: "text", text: header + pageHeader + summary + (nl > 0 ? trunc.slice(0, nl) : trunc) }] };
    }
    return { content: [{ type: "text", text: header + pageHeader + output }] };
});
// ── ready ─────────────────────────────────────────────────────────────────────
server.tool("ready", `Check whether the code search index is fully up to date with the file system.
...
Args:
  root: Named source root to check (empty = default root).`, { root: z.string().default("") }, async ({ root }) => {
    let collection;
    try {
        [collection] = getRoot(root);
    }
    catch (e) {
        return { content: [{ type: "text", text: `Error: ${e.message}` }] };
    }
    ...
    try {
        ...
        return { content: [{ type: "text", text: `Indexserver is NOT running: ${e.message}\nStart it with: ts start` }] };
    }
...
// ── verify_index ──────────────────────────────────────────────────────────────
server.tool("verify_index", `Verify that the code search index is up to date with the file system.
...
Args:
  ...
  delete_orphans: Remove entries for deleted files. Default true.`, {
    ...
    if (act === "stop") {
        ...
        if (status === 404)
            return { content: [{ type: "text", text: "No verification scan is currently running." }] };
        if (status !== 200)
            return { content: [{ type: "text", text: `Stop failed (${status}): ${data?.error ?? JSON.stringify(data)}` }] };
        return { content: [{ type: "text", text: "Verification scan stopped." }] };
    }
    if (act === "status") {
        const { status, data } = await apiGet("/status");
        if (status !== 200)
            return { content: [{ type: "text", text: `Status failed (${status}): ${data?.error ?? JSON.stringify(data)}` }] };
        const syncer = data.syncer ?? {};
        const prog = syncer.progress ?? {};
        if (!Object.keys(prog).length) {
            return { content: [{ type: "text", text: "No sync has been run yet. Use action='start' to begin." }] };
        }
        ...
        lines.push(`Status   : ${prog.status ?? "?"}`, `Phase    : ${prog.phase ?? "?"}`, `Started  : ${prog.started_at ?? "?"}`, `Updated  : ${prog.last_update ?? "?"}`, `FS files : ${prog.fs_files ?? "?"}`, `Index    : ${prog.index_docs ?? "?"} docs`, `Missing  : ${prog.missing ?? 0}`, `Stale    : ${prog.stale ?? 0}`, `Orphaned : ${prog.orphaned ?? 0}`, `Re-indexed: ${prog.updated ?? 0}`, `Deleted  : ${prog.deleted ?? 0} orphans removed`, `Errors   : ${prog.errors ?? 0}`);
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    if (act === "start") {
        let collection, srcRoot;
        try {
            [collection, srcRoot] = getRoot(root);
        }
        catch (e) {
            return { content: [{ type: "text", text: `Error: ${e.message}` }] };
        }
        ...
        if (status === 409)
            return { content: [{ type: "text", text: "A verification scan is already running.\nUse action='status' to monitor, or action='stop' to cancel." }] };
        if (status !== 200)
            return { content: [{ type: "text", text: `Start failed (${status}): ${data?.error ?? JSON.stringify(data)}` }] };
        return { content: [{ type: "text", text: `Verification scan started.\nRoot      : '${root || "default"}' → ${srcRoot}\nCollection: ${collection}\nUse action='status' to monitor progress.` }] };
    }
    return { content: [{ type: "text", text: `Unknown action: '${action}'. Use 'start', 'status', or 'stop'.` }] };
});
// ── manage_service ────────────────────────────────────────────────────────────
server.tool("manage_service", `Start, stop, restart, check status, or rebuild the code search service.
...
Args:
  action: One of:
          ...
          "rebuild" — Wipe the index and re-index everything from scratch.
                      Runs in the background; monitor with action='status'.`, { action: z.string().default("status") }, async ({ action }) => {
    const VALID = ["start", "stop", "restart", "status", "rebuild"];
    const act = action.toLowerCase().trim();
    if (!VALID.includes(act)) {
        return { content: [{ type: "text", text: `Unknown action: '${action}'. Valid: ${VALID.join(", ")}` }] };
    }
    if (act === "status") {
        try {
            const { status, data } = await apiGet("/status");
            ...
            for (const [name, info] of Object.entries(data.collections ?? {})) {
            ...
            if (data.syncer?.running)
                lines.push(`Syncer: running  phase=${data.syncer.progress?.phase ?? "?"}`);
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: `Indexserver not reachable: ${e.message}\nTry: manage_service(action='start')` }] };
        }
    }
    if (act === "rebuild") {
        const results = [];
    ...
    if (res.status !== 0) {
        return { content: [{ type: "text", text: `docker ${act} failed (exit ${res.status ?? "?"}): ${out}` }] };
    }
    return { content: [{ type: "text", text: out || `Service '${act}' completed.` }] };
});
// ── service_status ────────────────────────────────────────────────────────────
server.tool("service_status", `Check whether the Typesense code search service is running.
Returns server health, document count per root, and watcher state.
If not running, returns instructions to start it.

Args:
  root: Named root to inspect (empty = show all configured roots).`, { root: z.string().default("") }, async ({ root }) => {
    let st;
    try {
        ...
        return { content: [{ type: "text", text: `Indexserver is NOT running.\nStart it with: ts start\nError: ${e.message}` }] };
    }
