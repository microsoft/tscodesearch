...
async function queueWarning() {
    try {
        ...
        if (indexerRunning)
            parts.push("indexer walk in progress");
        if (parts.length)
...
Examples:
  ...
  query_single_file("accesses_on", "IDataStore", file="$SRC_ROOT/services/DataManager.cs")`, {
    ...
    const outLines = fileResult.matches.map((m) => `${rel}:${m.line}: ${(m.text ?? "").trimEnd()}`);
    const output = outLines.join("\n");
    if (output.length > MAX_OUTPUT_CHARS) {
        const trunc = output.slice(0, MAX_OUTPUT_CHARS);
        const nl = trunc.lastIndexOf("\n");
        ...
        return { content: [{ type: "text", text: header + summary + (nl > 0 ? trunc.slice(0, nl) : trunc) }] };
    }
...
Args:
  ...
  delete_orphans: Remove entries for deleted files. Default true.`, {
    ...
    if (act === "status") {
        ...
        if (status === 404)
            return { content: [{ type: "text", text: "No verification scan has been run. Use action='start' to begin." }] };
        if (status !== 200)
        ...
        lines.push(`Status   : ${data.status ?? "?"}`, `Phase    : ${data.phase ?? "?"}`, `Started  : ${data.started_at ?? "?"}`, `Updated  : ${data.last_update ?? "?"}`, `FS files : ${data.fs_files ?? "?"}`, `Index    : ${data.index_docs ?? "?"} docs`, `Missing  : ${data.missing ?? 0}`, `Stale    : ${data.stale ?? 0}`, `Orphaned : ${data.orphaned ?? 0}`, `Re-indexed: ${data.updated ?? 0}`, `Deleted  : ${data.deleted ?? 0} orphans removed`, `Errors   : ${data.errors ?? 0}`);
        return { content: [{ type: "text", text: lines.join("\n") }] };
    }
    if (act === "start") {
  ...
  action: One of:
    ...
    if (act === "status") {
        try {
            ...
            if (data.indexer?.running)
                lines.push(`Indexer: running  ${JSON.stringify(data.indexer.progress ?? {})}`);
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (e) {
