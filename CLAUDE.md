...
**Never introduce non-ASCII characters in code, comments, docstrings, output strings, or docs.** The Windows console encoding (`cp1252`) and many downstream tools (LLM consumers, terminal renderers, log aggregators) misrender or fail outright on Unicode codepoints that look harmless in an editor. The whole codebase has been normalised to ASCII; keep it that way.

When you're tempted to reach for a "pretty" character, use these ASCII equivalents (the table refers to codepoints by name to avoid embedding the very characters this file forbids):

| Tempting Unicode | Use ASCII instead |
|------------------|-------------------|
...
Run `python -m scripts.find_nonascii` from the repo root to verify no non-ASCII codepoint has slipped in (it categorises by file region: docstring, string literal, comment, code). `python -m scripts.replace_nonascii --apply` will mass-rewrite the known codepoints if anything sneaks back in.
