...
| `src/test/integration.test.ts` | 23 integration tests. Requires Typesense running (`ts start`). Creates an isolated `codesearch_tstest_{timestamp}` collection, upserts a realistic `WidgetService.cs` document, polls until indexed, then exercises all modes plus extension/folder-path filters. Cleans up on completion. |
