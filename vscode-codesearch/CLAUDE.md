...
**Default Tantivy tokenizer splits on punctuation.** `Task<Widget>` → `task`, `widget` automatically. No special tokenizer setup is needed and the schema is fixed at index creation time. After a schema change, `ts index --resethard` is the way to rebuild.
